...
def main(model_config,
    ...
    for train_set, val_set in zip(train_sets, val_sets):
        ...
        score_accuracy_top3, _, score_precision, _ = model.eval_fn(val_loader, device)
...
if __name__ == '__main__':
    ...
    When calling this function from the command line the "configuration" parameter must include the JSON filename
    which includes the optimal configuration. This file is output by the AutoML.py script. Note that it has no the name
    of "opt_cfg" but "opf_cfg_moreDescriptiveInfo". These more info include the random seed which can be passed as
    parameter in --seed to obtain comparable results.
    ...
    cmdline_parser.add_argument('-c', '--configuration',
                                help='JSON file with the optimal configuration',
                                type=str)
    ...
    main(
        ...
        batch_size=opt_cfg.get("batch_size", 290),
        learning_rate=opt_cfg.get("learning_rate_init", 0.0016651181400389517),
