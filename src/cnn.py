...
class torchModel(nn.Module):
    ...
    """
    def __init__(self, config, input_shape=(1, 28, 28), num_classes=10, seed=0):
        ...
        with torch.no_grad():  # no gradient needed
            for images, labels in loader:
                ...
                precision = precision_score(labels.data.cpu().detach().numpy(),
                                            np.argmax(outputs.data.cpu().detach().numpy(), axis=1), labels=np.arange(0, 17).tolist(), average=None, zero_division=0)
                ...
                y_predicted = np.argmax(outputs.data.cpu().detach().numpy(), axis=1)
                y_target = labels.data.cpu().detach().numpy()
                ok_predicted = y_predicted == y_target
                precitions.extend(ok_predicted)

        return score.avg, score_top5.avg, score_precision.avg, precitions
